...
class CAM_DGD():
    ...
    def do_train(self, model, dataloader, return_epoch_results=False):
        ...
        # 0: CAM_DGD model, 1: Homo GD, 2: Hetero GD
        ...
        while True:
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    # ort loss
                    cosine_similarity_s_c_l = self.cosine(output['s_l'].reshape(-1, 30), output['c_l'].reshape(-1, 30), torch.tensor([-1]).cuda())
                    cosine_similarity_s_c_v = self.cosine(output['s_v'].reshape(-1, 30), output['c_v'].reshape(-1, 30), torch.tensor([-1]).cuda())
                    cosine_similarity_s_c_a = self.cosine(output['s_a'].reshape(-1, 30), output['c_a'].reshape(-1, 30), torch.tensor([-1]).cuda())
            ...
            if isBetter:
                ...
                # save model
                model_save_path = './pt/CAM_DGD.pth'
