...
class MMDataset(Dataset):
    def __init__(self, args, mode='train', context_image_features_path=None):
        self.mode = mode
        self.args = args
        self.context_image_features_path = context_image_features_path

        if self.context_image_features_path:
            with open(self.context_image_features_path, 'rb') as f:
                self.context_image_features = pickle.load(f)  # Load the tuple (context_frames, context_depth_maps)
        else:
            self.context_image_features = None  # If no path is provided, set to None
    ...
    def __init_mosi(self):
        ...
        if self.context_image_features is not None:
            context_frames, context_depth_maps = self.context_image_features
            # Example: Concatenate the context frames and depth maps with existing vision features
            self.vision = np.concatenate((self.vision, context_frames, context_depth_maps), axis=-1)  # Adjust based on your feature shapes
...
def MMDataLoader(args, num_workers):
    ...
    datasets = {
        'train': MMDataset(args, mode='train', context_image_features_path=".\\dataset\\MOSEI\\Processed\\context_features.pkl"),
        'valid': MMDataset(args, mode='valid', context_image_features_path=".\\dataset\\MOSEI\\Processed\\context_features.pkl"),
        'test': MMDataset(args, mode='test', context_image_features_path=".\\dataset\\MOSEI\\Processed\\context_features.pkl")
    }
