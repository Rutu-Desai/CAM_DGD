...
from trains.singleTask.model import CAM_DGD
...
def _run(args, num_workers=4, is_tune=False, from_sena=False):
    ...
    if args.is_distill:
        ...
        model = []
        model_CAM_DGD = getattr(CAM_DGD, 'CAM_DGD')(args)
        ...
        model_CAM_DGD, model_distill_homo, model_distill_hetero = model_CAM_DGD.cuda(), model_distill_homo.cuda(), model_distill_hetero.cuda()

        model = [model_CAM_DGD, model_distill_homo, model_distill_hetero]
    else:
        print("testing phase for CAM_DGD")
        model = getattr(CAM_DGD, 'CAM_DGD')(args)
        model = model.cuda()
    ...
    if args.mode == 'test':
        ...
        model[0].load_state_dict(torch.load('pt/CAM_DGD.pth'))
